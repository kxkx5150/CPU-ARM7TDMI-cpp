...
class IO {
  ...
  public:
    ...
    const uint8_t ws0_s_lut[2] = {2, 1};
    const uint8_t ws1_s_lut[2] = {4, 1};
    const uint8_t ws2_s_lut[2] = {8, 1};
    const uint8_t ws_n_lut[4]  = {4, 3, 2, 8};

  public:
    IO(GBA *_gba);

    uint8_t io_read(uint32_t address);
    void    dma_load(uint8_t ch, uint8_t value);
    void    tmr_load(uint8_t idx, uint8_t value);
    void    snd_reset_state(uint8_t ch, bool enb);
